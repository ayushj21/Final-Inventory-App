...
public class InvProvider extends ContentProvider {
    ...
    static {
        sUriMatcher.addURI(DatabaseContract.CONTENT_AUTHORITY, DatabaseContract.PATH_INVENTORY, ITEMS);
        sUriMatcher.addURI(DatabaseContract.CONTENT_AUTHORITY, DatabaseContract.PATH_INVENTORY + "/#", ITEM_ID);
    }
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        switch (match) {
            case ITEMS:
                ...
                cursor = database.query(DatabaseEntry.TABLE_NAME, projection, selection,
    ...
    private Uri insertItem(Uri uri, ContentValues values) {
        ...
        long id = db.insert(DatabaseEntry.TABLE_NAME, null, values);
        if (id == -1) {
...
//        return ContentUris.withAppendedId(uri, id);
    ...
    private int updateItem(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (values.containsKey(DatabaseEntry.COLUMN_INVENTORY_PRODUCT_NAME)) {
            String name = values.getAsString(DatabaseEntry.COLUMN_INVENTORY_PRODUCT_NAME);
            if (name == null) {
        ...
        if (values.containsKey(DatabaseEntry.COLUMN_INVENTORY_PRICE)) {
            Integer price = values.getAsInteger(DatabaseEntry.COLUMN_INVENTORY_PRICE);
            if (price != null && price < 0) {
        ...
        if (values.containsKey(DatabaseEntry.COLUMN_INVENTORY_QUANTITY)) {
            Integer quantity = values.getAsInteger(DatabaseEntry.COLUMN_INVENTORY_QUANTITY);
            if (quantity != null && quantity < 0) {
        ...
        if (values.containsKey(DatabaseEntry.COLUMN_INVENTORY_SUPPLIER_NAME)) {
            String supplierName = values.getAsString(DatabaseEntry.COLUMN_INVENTORY_SUPPLIER_NAME);
            if (supplierName == null) {
        ...
        if (values.containsKey(DatabaseEntry.COLUMN_INVENTORY_SUPPLIER_PHONE)) {
            Integer supplierPhone = values.getAsInteger(DatabaseEntry.COLUMN_INVENTORY_SUPPLIER_PHONE);
            if (supplierPhone != null && supplierPhone < 0) {
        ...
        int rowsUpdated = database.update(DatabaseEntry.TABLE_NAME, values, selection, selectionArgs);
